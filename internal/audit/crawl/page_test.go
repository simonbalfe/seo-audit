...
<body><main><article><h1>Useful heading</h1><p>This useful paragraph explains the page clearly for readers.</p><a href="/next">Next</a><a href="tel:+442012345678">Call</a><a href="/book-appointment">Book appointment</a><img src="/image.jpg" alt=""></article></main></body>
</html>`))
	}))
	defer server.Close()
	...
	if len(report.InternalLinks) != 2 || report.InternalLinks[0] != server.URL+"/next" {
		t.Fatalf("unexpected links %#v", report.InternalLinks)
	}
	if len(report.PhoneLinks) != 1 || report.PhoneLinks[0] != "+442012345678" {
		t.Fatalf("unexpected phone links %#v", report.PhoneLinks)
	}
	if len(report.BookingLinks) != 1 || report.BookingLinks[0].URL != server.URL+"/book-appointment" {
		t.Fatalf("unexpected booking links %#v", report.BookingLinks)
	}
}

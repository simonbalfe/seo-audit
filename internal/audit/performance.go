...
func (c *Client) inspectPerformance(ctx context.Context, report SiteReport, options Options) PerformanceReport {
	...
	emitProgress(options, "performance", "Selected %d representative pages for %s testing", len(targets), performanceProfile)
	...
	for index, target := range targets {
		emitProgress(options, "performance", "Testing %d/%d: %s", index+1, len(targets), target)
		...
		if err != nil {
			...
			emitProgress(options, "performance", "Could not test %s: %v", target, err)
			continue
		}
